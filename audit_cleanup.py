...
def analyze_code_structure(project_root: Path) -> Dict[str, any]:
    ...
    structure = {
        ...
        "frontend": {
            ...
            "package_json": (
                (frontend / "package.json").exists() if frontend.exists() else False
            ),
        },
    }
...
def main():
    ...
    if structure["backend"]["exists"]:
        ...
        print(
            f"  Entry point: {'✓' if structure['backend']['main_entry'] else '✗'} main.py"
        )
        print(f"  Modules: {', '.join(structure['backend']['app_modules'][:5])}")
        if len(structure["backend"]["app_modules"]) > 5:
            print(
                f"           ... and {len(structure['backend']['app_modules']) - 5} more"
            )
    ...
    if structure["frontend"]["exists"]:
        ...
        print(
            f"  Structure: {'✓' if structure['frontend']['src_exists'] else '✗'} src/"
        )
        print(
            f"  Config: {'✓' if structure['frontend']['package_json'] else '✗'} package.json"
        )
    else:
    ...
    md_files = [
        f for f in md_files if "node_modules" not in str(f) and ".git" not in str(f)
    ]

    root_docs = [f for f in md_files if f.parent == project_root]
    docs_dir_docs = [f for f in md_files if "docs/" in str(f)]
    backend_docs = [
        f for f in md_files if "backend/" in str(f) and "backend/docs" not in str(f)
    ]
    ...
    if len(root_docs) > 15:
        print_warning(
            "Many documentation files in root (consider organizing into /docs)"
        )
    else:
    ...
    if artifacts:
        recommendations.append(
            f"Clean {len(artifacts)} build artifacts (safe to delete)"
        )
    ...
    if len(root_docs) > 15:
        recommendations.append(
            "Consider organizing root-level docs into /docs directory"
        )
