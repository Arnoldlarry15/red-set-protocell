...
def analyze_session(filename: str):
    ...
    print(f"Concentration episodes (effective_rank < 2.0): {concentration_count}/{len(effective_ranks)}")
    ...
    for log in logs:
        ...
        weights_no_bias = [max(0.1, c['weight_without_behavior']) for c in candidates]
        
        # Winner WITHOUT behavior bias
        winner_idx_no_bias = weights_no_bias.index(max(weights_no_bias))
        winner_without_bias = candidates[winner_idx_no_bias]
        
        # True flip: different winners
        if winner_with_bias['strategy'] != winner_without_bias['strategy']:
            flip_count += 1
    ...
    for log in logs:
        ...
        assert abs(probs_sum - 1.0) < 1e-6, f"Round {log['round']}: probs sum to {probs_sum}"
    ...
    max_persistence = max(trait_counts.values()) / window_size if trait_counts else 0.0
    
    print(f"\nMax trait persistence (any 20-round window): {max_persistence:.0%}")
    
    if max_persistence > 0.60:
        most_persistent = max(trait_counts, key=trait_counts.get)
        print(f"→ WARNING: {most_persistent} triggering >{max_persistence:.0%} of rounds")
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        ...
        results = analyze_session(sys.argv[1])
    else:
        ...
        if len(all_results) == 3:
            ...
            for metric in ['mean_effective_rank', 'flip_rate', 'avg_flip_magnitude']:
                print(f"\n{metric}:")
