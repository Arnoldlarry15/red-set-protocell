...
def analyze_session(filename: str):
    ...
    print(
        f"Concentration episodes (effective_rank < 2.0): "
    ...
    print(f"\nMax trait persistence (any 20-round window): " f"{max_persistence:.0%}")

    if max_persistence > 0.60:
        most_persistent = max(trait_counts, key=trait_counts.get)
        print(
            f"→ WARNING: {most_persistent} triggering "
