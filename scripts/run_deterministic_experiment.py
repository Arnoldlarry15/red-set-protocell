...
async def main():
    ...
    parser.add_argument(
        "--seed", type=int, default=15, help="Primary seed for experiment (default: 15)"
    )
    parser.add_argument(
        "--rounds",
        type=int,
    ...
    seeds = {"A_low": args.seed, "B_mid": 1337, "C_high": 9001}
    ...
    print(f"{'='*60}")
    print(
        f"Total rounds executed: {sum(s['session']['total_rounds'] for s in all_stats.values())}"
    )
    print(f"\nOutput files:")
