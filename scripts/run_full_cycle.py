    ...
    async def run_full_cycle(self) -> Dict[str, Any]:
        ...
        self.audit_trail["configuration"] = {
            "backend": (
                ...
                else str(config.target.backend)
            ),
        ...
        for round_num in range(1, self.rounds + 1):
            ...
            try:
                # Get prior metadata
                prior_metadata = (
                    await orchestrator.state_manager.get_prior_rounds_async(limit=10)
                )
                ...
                sniper_instruction = {
                    ...
                    "input_context": {
                        ...
                        "evolution_pool_size": (
                            len(orchestrator.sniper.evolution_pool)
                            if hasattr(orchestrator.sniper, "evolution_pool")
                            else 0
                        ),
                    },
                }
                self.audit_trail["role_separation"]["sniper_instructions"].append(
                    sniper_instruction
                )

                # Sniper generates prompt
                prompt, attack_domain = await orchestrator.sniper.generate_prompt(
                    prior_metadata
                )
                ...
                if not is_allowed:
                    ...
                    round_detail = {
                        ...
                        "egg_blocked": True,
                        "blocked_category": (
                            blocked_info.category if blocked_info else None
                        ),
                ...
                self.audit_trail["role_separation"]["target_interactions"].append(
                    target_interaction
                )

                # Target executes prompt
                target_response = await orchestrator.target.execute(
                    prompt, metadata={"round": round_num, "domain": attack_domain.value}
                )
                ...
                self.audit_trail["role_separation"]["spotter_instructions"].append(
                    spotter_instruction
                )
        ...
        self.audit_trail["statistics"] = {
            ...
            "successful_rounds": len(
                [
                    r
                    for r in self.audit_trail["round_details"]
                    if not r.get("egg_blocked") and not r.get("error")
                ]
            ),
            "blocked_rounds": len(
                [r for r in self.audit_trail["round_details"] if r.get("egg_blocked")]
            ),
            "error_rounds": len(
                [r for r in self.audit_trail["round_details"] if r.get("error")]
            ),
        ...
        output_file = (
            self.output_dir
            / f"full_cycle_seed_{self.seed}_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.json"
        )
        ...
        print(
            f"Successful rounds: {self.audit_trail['statistics']['successful_rounds']}"
        )
...
async def verify_determinism(seed: int, rounds: int = 10) -> bool:
    ...
    # Run 1
    print("=== RUN 1 ===")
    runner1 = FullCycleRunner(
        seed=seed, rounds=rounds, output_dir="full_cycle_logs/verify_run1"
    )
    ...
    # Run 2
    print("=== RUN 2 ===")
    runner2 = FullCycleRunner(
        seed=seed, rounds=rounds, output_dir="full_cycle_logs/verify_run2"
    )
