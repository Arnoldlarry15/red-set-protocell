...
def check_script(filepath, script_name):
    ...
    print(
        f"✓ Does NOT import get_default_config: {'YES' if not has_get_default else 'NO'}"
    )
...
def main():
    ...
    scripts_to_check = [
        (
            f"{base_path}/scripts/run_deterministic_experiment.py",
            "run_deterministic_experiment.py",
        ),
        ...
        (
            f"{base_path}/backend/examples/time_analytics.py",
            "examples/time_analytics.py",
        ),
    ]
