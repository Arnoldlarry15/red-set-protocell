    ...
    async def run_verification(self) -> bool:
        ...
        for i in range(1, self.iterations + 1):
            ...
            result = {
                ...
                "round_scores": [
                    r.get("global_score", 0.0) for r in audit["round_details"]
                ],
            }
        ...
        for round_idx in range(self.rounds):
            round_scores = [
                (
                    r["round_scores"][round_idx]
                    if round_idx < len(r["round_scores"])
                    else None
                )
            ...
            if len(unique_round_scores) == 1:
                print(
                    f"  Round {round_idx + 1}: [OK] Identical ({list(unique_round_scores)[0]:.3f})"
                )
            else:
